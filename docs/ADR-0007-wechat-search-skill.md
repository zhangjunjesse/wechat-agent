...
- **群聊**：`chat_roster` 里该用户是某个 `chat_wxid` 的成员——**分级匹配，不是并列 OR**（ADR-0032，2026-09-16 修订）：wxid 已知时只认 wxid，昵称完全不参与；只有 wxid 缺失时才降级用昵称匹配，且降级路径撞见同一昵称对应多个不同真实 `member_wxid`（生产实测：4 个不同已核验用户共享昵称"Z.俊"）时拒绝返回，而不是返回两人的并集。原始设计曾是"wxid 或昵称任一命中即可"的并列 OR，这在 wxid 缺失（当时生产全部 7 个已核验用户皆如此）时会让同名不同人互相看到对方的全部群聊——已改，细节与验收见 ADR-0032。
- **与助手的私聊**：从被同步账号（即"助手"本体）的视角看，"我和用户 X 的私聊" 在 `messages` 表里就是 `chat_wxid === X 自己的 wxid`——所以用户自己的 wxid 本身就是他们私聊线程的标识，不需要花名册，也不需要额外配置。这一点用真实数据核对过（服务器上 `chat_wxid='zj391504704'` 对应昵称"Z.俊"的私聊，`sender_wxid` 与之一致）。
  - **2026-09-19 修正：线程的显示名取真实 `chat_display`，不再写死标签。** 原实现把这条私聊的名字硬编码成 `'与助手的对话（私聊）'`，于是 `wechat_search_chat("Z.俊")` 解析失败（`chat_not_found_or_not_accessible`）——可同步库里这条私聊的 `chat_display` 恰恰就是这个用户自己的昵称（用户在自己的客户端里看到的就是这条会话）。生产实测：修复前 `searchChat('Z.俊')` 命中 0 条，修复后 `'Z.俊'`／`'助手'`／`'私聊'`／wxid 四种问法全部命中同一条线程（117 条）。无同步记录时仍回落到 `'与助手的对话（私聊）'`（该别名保留在 `alias` 字段，工具入口不变）。
  - **降级路径（wxid 未知）现在也能解析到这条线程**：按 `chat_display === nickname` 精确定位。这不违反 ADR-0032 的歧义拒绝——那条规则防的是 `chat_roster` 里"同一昵称对应多个 `member_wxid`、因而可能看到别人的群"；而私聊线程不是共享资源，它就是该用户自己的 1:1 会话，精确等值匹配不存在跨用户泄漏面。ADR-0032 未改变群聊一侧的分级匹配。
...
- `tests/wechat-private-chat.test.mjs`（2 条，2026-09-19）：私聊显示名取真实 `chat_display`（钉住"昵称即私聊名"这一生产事实，防止回退到写死标签）、`'Z.俊'`／`'助手'`／`'私聊'`／wxid 四种入口解析到同一条线程、无 wxid 时按昵称降级仍可解析。整体 `npm test` 534/534。
