...
- **真实用户路径未走过**：探针直连 `AgentsSdkAgent`，没有经过 `MessageRouter`
  → iLink 真实收发这一段。"用户在微信里打『切换到专家模式』然后真的收到确认"
  这条端到端路径，要等上线后由真人跑一次。
- **生产上线后的首次切换未观察**：`data/expert-mode.json` 在生产从未被创建过，
  首次写盘（目录权限、持久卷挂载）未经真实验证。
