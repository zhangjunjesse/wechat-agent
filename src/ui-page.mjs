...
.qrwrap{margin-top:20px;display:flex;flex-direction:column;align-items:center;gap:14px}
.qrwrap img{width:218px;height:218px;object-fit:contain;border-radius:14px;box-shadow:0 12px 30px #1a2a5a16}
.qrwrap .cap{color:var(--muted);font-size:13px}
...
.overlay{position:fixed;inset:0;background:#0b122080;backdrop-filter:blur(4px);display:none;align-items:center;justify-content:center;z-index:50}
.modal{background:#fff;border-radius:22px;width:min(420px,90vw);padding:34px 30px;text-align:center;box-shadow:0 30px 70px #0006}
.modal .ok-ic{width:76px;height:76px;border-radius:50%;background:#ecfdf3;border:2px solid #abefc6;display:grid;place-items:center;margin:0 auto 18px;font-size:38px;color:var(--ok);animation:pop .4s cubic-bezier(.2,1.4,.4,1)}
@keyframes pop{from{transform:scale(.6);opacity:0}to{transform:scale(1);opacity:1}}
.modal h2{margin:0 0 6px;font-size:21px;color:var(--ink)}
.modal p{color:var(--muted);margin:0 0 4px;font-size:14.5px}
.modal .nick{color:var(--ink);font-weight:800;font-size:18px}
.modal .btn{margin-top:22px;width:100%;}
.modal .done{display:inline-block;background:#f1f3f9;color:var(--muted);border-radius:30px;padding:6px 13px;font-size:12.5px;margin-top:16px}
...
<div class="foot">身份资料仅用于匹配你的微信记录 · 未核验用户不会使用 AI 服务</div></div>
<div class="overlay" id="overlay"><div class="modal"><div class="ok-ic">✓</div><h2>身份核验成功</h2><p>欢迎你，<span class="nick" id="modalNick"></span></p><p>你的微信身份已确认，现在可以开始使用个人助手了。</p><button class="btn" id="modalOk">开始使用</button><div class="done">请回到微信，向「助手」发送消息即可对话</div></div></div><script>
...
async function pollVerify(user){if(!verifyId)return;const r=await fetch('api/profile-verifications/'+verifyId,{headers:{'x-user-id':user}});if(!r.ok)return;const x=await r.json();if(x.status==='verified'){const nick=x.profile.nickname||'朋友';$('hint').textContent='✓ 已核验昵称：'+nick;$('hint').classList.add('success');$('chat').classList.add('on');clearInterval(verifyTimer);$('modalNick').textContent=nick;$('overlay').style.display='flex';return}if(x.status==='expired'){$('hint').textContent='验证码已过期，请返回重新绑定。';$('hint').classList.add('warn');clearInterval(verifyTimer)}}
$('modalOk').onclick=()=>$('overlay').style.display='none';
...
async function pollBind(user,id){const r=await fetch('api/bindings/'+id,{headers:{'x-user-id':user}});const b=await r.json();if(!r.ok){setStatus('绑定状态查询失败，请重试。',null,'!');return}if(b.status==='bound'&&b.sessionExpired){setStatus('⚠️ 该 Bot 会话已过期，微信消息可能无法送达。请刷新页面重新绑定。','warn','!');return}const label=STATUS_LABEL[b.status]||b.status;setStatus(label,b.status==='bound'?'ok':b.status==='scanned'?'':'pulse',b.status==='bound'?'✓':'⟳');if(b.status==='bound'){startVerify(user,b);clearInterval(bindTimer)}}
$('start').onclick=async()=>{clearInterval(bindTimer);clearInterval(verifyTimer);verifyId=null;hasBound=false;$('verify').style.display='none';$('preVerify').style.display='block';$('chat').classList.remove('on');const user='u_'+crypto.randomUUID().slice(0,12);try{sessionStorage.setItem('wa-user',user)}catch(e){}setStatus('正在为你生成二维码…',null,'⟳');const r=await fetch('api/bindings',{method:'POST',headers:{'x-user-id':user},body:'{}'});const b=await r.json();if(!r.ok){setStatus('绑定失败：'+(b.error||'请重试'),'warn','!');return}$('botQr').src='';const q=await fetch('api/qr?payload='+encodeURIComponent(b.qrPayload));if(q.ok){$('qrwrap').style.display='flex';$('botQr').src=(await q.json()).dataUrl}setStatus('请使用微信扫描二维码，并在手机上确认登录',null,'⟳');bindTimer=setInterval(()=>pollBind(user,b.id),2000)};
</script></body></html>`
}
