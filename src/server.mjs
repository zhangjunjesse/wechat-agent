...
const sessionStore = new SessionStore({ file: process.env.SESSIONS_FILE || 'data/sessions.db', maxTurns: Number(process.env.SESSION_MAX_TURNS || 40) })
const historyProvider = process.env.WECHAT_SYNC_ACCESS_KEY ? async (userId, profile) => verifier?.recentForProfile(profile) : null
const agent = process.env.OPENAI_API_KEY ? (process.env.AGENT_SDK === 'openai' ? new AgentsSdkAgent({ model: process.env.OPENAI_MODEL || 'gpt-4o-mini', baseUrl: process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1', apiKey: process.env.OPENAI_API_KEY, historyProvider, sessionStore }) : new OpenAICompatibleAgent({ baseUrl: process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1', apiKey: process.env.OPENAI_API_KEY, model: process.env.OPENAI_MODEL || 'gpt-4o-mini', contextProvider: async (userId) => profileStore.get(userId), historyProvider, sessionStore })) : undefined
