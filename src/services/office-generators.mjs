...
function zip(files) {
  ...
  const cd = Buffer.concat(central); const count = Object.keys(files).length; const e = Buffer.alloc(22); e.writeUInt32LE(0x06054b50, 0); e.writeUInt16LE(0, 4); e.writeUInt16LE(0, 6); e.writeUInt16LE(count, 8); e.writeUInt16LE(count, 10); e.writeUInt32LE(cd.length, 12); e.writeUInt32LE(offset, 16); return Buffer.concat([...locals, cd, e])
}
