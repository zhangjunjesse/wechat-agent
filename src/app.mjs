...
function html(res, status, body) {
  res.writeHead(status, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store, no-cache, must-revalidate' })
  res.end(body)
}
...
function page() {
  ...
  <section id="verify" style="display:none;margin-top:28px;border-top:1px solid #ddd;padding-top:18px"><h2>第二步：添加微信“助手”并验证昵称</h2><p>请用微信扫描下方二维码，添加联系人 <b>助手</b>。添加后，请向助手发送页面上醒目标出的 6 位数字验证码。</p><img src="assistant-qr.jpg?v=2" alt="助手微信二维码" style="width:320px;height:auto;display:block;background:#fff" onload="document.getElementById('qrError').style.display='none'" onerror="document.getElementById('qrError').style.display='block'"><div id="qrError" style="display:none;color:#b42318">助手二维码暂时无法加载，请联系管理员。</div><p id="verifyHint">验证码功能将在绑定成功后启用。</p></section>
  <script>
  ...
  $('start').onclick=async()=>{clearInterval(timer);const user=$('user').value.trim();if(!user)return;
    ...
    timer=setInterval(async()=>{const x=await fetch('api/bindings/'+b.id,{headers:{'x-user-id':user}});const s=await x.json();$('status').textContent='状态：'+s.status+(s.profile?'\\n用户：'+(s.profile.nickname||'')+' '+(s.profile.username||''):'');if(s.status==='bound'){$('verify').style.display='block';if(!window.verifyId){const v=await fetch('api/profile-verifications',{method:'POST',headers:{'x-user-id':user,'content-type':'application/json'},body:JSON.stringify({ilinkUserId:s.profile?.providerUserId||s.providerBotId})});if(v.ok){const t=await v.json();window.verifyId=t.id;$('verifyHint').textContent='请向微信“助手”发送下面 6 位数字：\\n\\n'+t.code;$('verifyHint').style.cssText='white-space:pre-wrap;background:#fff7ed;border:2px solid #f97316;padding:16px;font-size:24px;font-weight:700;color:#9a3412';setInterval(async()=>{const z=await fetch('api/profile-verifications/'+t.id,{headers:{'x-user-id':user}});if(z.ok){const q=await z.json();if(q.status==='verified'){$('verifyHint').textContent='已核验昵称：'+q.profile.nickname+'\\nwxid：'+q.profile.wxid;$('chat').style.display='block'}}},3000)}else{const e=await v.json().catch(()=>({}));$('verifyHint').textContent='生成验证码失败：'+(e.error||v.status)}}}}if(['bound','expired','failed'].includes(s.status))clearInterval(timer)},2000)
  };
