...
export function createApp({ provider, agent = { async respond({ text }) { return { text: `Echo: ${text}` } } }, clock, pollIntervalMs, store, verifier, profileStore }) {
  ...
  const verification = verifier ? new VerificationService({ verifier, store: profileStore, onVerified: async ({ userId, task, profile }) => { const binding = owned.find((x) => x.userId === userId && x.status === 'bound'); if (binding?.providerBotId && task.ilinkUserId) await provider.sendText({ providerBotId: binding.providerBotId, toProviderUserId: task.ilinkUserId, text: `身份验证成功！你好，${profile.nickname || '朋友'}。现在可以开始使用个人助手了。`, contextToken: task.contextToken || '' }).catch((error) => console.error('[verification-reply]', error.message)) } }) : null
...
$('send').onclick=async()=>{const text=$('chattext').value.trim();if(!text)return;$('chattext').value='';$('chatlog').innerHTML+='<div><b>我：</b>'+text.replaceAll('<','&lt;')+'</div>';const r=await fetch('api/chat',{method:'POST',headers:{'x-user-id':$('user').value,'content-type':'application/json'},body:JSON.stringify({text})});const x=await r.json();$('chatlog').innerHTML+='<div><b>Agent：</b>'+((x.text||x.message||x.error||'').replaceAll('<','&lt;'))+'</div>'};</script>` }
