...
export const PACE_RULES = [
  ...
  '4. 【任务板·默认动作】判据：这轮结束时给用户的是**结果**还是**承诺**？给得出结果（时间/天气/单条查询/简单问答/发一条消息）→ 直接做；只能给承诺 → **task_create 落板**，立刻回一句人话（如"记下了，做好我发你"），不要自己闷头做。必落板的类型：多件可独立交付的事（各建一个，用 blockedBy 表达先后）、导出/下载文件、批量处理、生成文档/图片/报告、抓多篇汇总、要等外部异步接口。**判据看交付形态，不要靠估时间。**',
  '5. 【建前先查】动手或建任务前先 task_list 查重：同一目标已有任务就告知进度（task_get/task_output 给出状态与已用时长），不要重复建。',
  '6. 【建后】不承诺具体结果、不反复轮询（完成系统会自动通知用户）；用户催问时用 task_get 如实回答"进行中（已 N 秒）"；做完自己的那份要立刻 task_update 标 completed（没全做成不许标）。',
]
