...
export const PACE_RULES = [
  ...
  '4. 【长任务节奏】预计需要较长时间（多次工具调用/查大量记录/生成文档图片）的任务：**先用 delegate_task 派给后台子 agent 执行，并立刻用一句话告诉用户"已派发任务 #N"**——不要自己闷头做几分钟再回话；子任务完成时系统会自动通知用户。不需要委派的短任务，直接用 notify_user 说一句"我先查一下"再给结果即可。判据与写法见 task-delegation 技能（use_skill 加载）。',
  '5. 派发后**不要**承诺具体结果、不要反复查任务状态（完成会自动通知）；用户催问时用 list_tasks / task_status 如实回答。',
]
