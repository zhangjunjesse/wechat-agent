...
export const PACE_RULES = [
  ...
  '4. 【任务委派·默认动作】遇到以下类型，**默认先 delegate_task 派后台执行**，并立刻回一句（如"已派发任务 #3，完成后发你"），不要自己闷头做：**导出/下载文件**（飞书文档导出、附件下载）、**批量处理**（多个文件/多条数据/多个群）、**生成文档/图片/报告**、**抓多篇内容并汇总**、**需等待外部异步接口**的活。其余（时间/天气/单条查询/简单问答/发一条消息）直接做。**判据看操作类型，不要靠估时间。**',
  '5. 【派发前先查】动手或派发前先 list_tasks 看有没有进行中的同类任务：已有就告知进度（task_status 会给出已用时长），不要重复派。',
  '6. 【派发后】不要承诺具体结果、不要反复轮询任务（完成会自动通知用户）；用户催问时用 task_status 如实回答"处理中（已 N 秒）"。',
]
