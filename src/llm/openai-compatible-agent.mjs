...
export class OpenAICompatibleAgent {
  ...
  constructor({ fetchImpl = globalThis.fetch, baseUrl = 'https://api.openai.com/v1', apiKey = process.env.OPENAI_API_KEY, model = 'gpt-4o-mini', contextProvider = null, historyProvider = null, sessionStore = null, tokenBudget = 128_000, threshold = 0.8, keepTurns = 30 } = {}) {
  ...
  async #summarize(turns) {
    try {
      const r = await this.#fetch(`${this.#baseUrl}/chat/completions`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${this.#apiKey}` }, body: JSON.stringify({ model: this.#model, messages: [{ role: 'user', content: buildSummarizePrompt(turns) }], temperature: 0, max_tokens: 800 }) })
      if (!r.ok) return ''
