...
export class AgentsSdkAgent {
  ...
  constructor({ model, baseUrl = 'https://api.openai.com/v1', apiKey = process.env.OPENAI_API_KEY, historyProvider = null, sessionStore = null, tokenBudget = 128_000, threshold = 0.8, keepTurns = 30 }) {
  ...
  async respond({ userId, text, profile }) {
    ...
    // Append to full transcript, then fold if over the token threshold.
    let { transcript, summary } = this.#sessions.append(userId, text, answer)
    if (this.#compactor.needsFold(transcript)) {
      const folded = await this.#compactor.fold(transcript, session.summary)
      const saved = this.#sessions.fold(userId, folded.summary, folded.keptTranscript)
...
// expose token estimator for reuse in tests/health
