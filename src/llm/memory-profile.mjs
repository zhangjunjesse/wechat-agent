...
export const PROFILE_MAX_CHARS = Number(process.env.PROFILE_MAX_CHARS || 800)
...
3. 每段 1-5 条要点；该段没有内容就写"（暂无）"。
4. 数字/日期/专有名词原样保留。
5. 总长不超过 ${PROFILE_MAX_CHARS} 字。
...
export class MemoryProfiler {
  ...
  async generate(store, userId, now = Date.now()) {
    ...
    let raw = ''
    try {
      raw = await this.#complete([{ role: 'user', content: buildProfilePrompt(cards, now) }], { temperature: 0, maxTokens: 1200 })
    } catch (e) {
      return { ok: false, cards: cards.length, reason: `llm error: ${String(e).slice(0, 120)}` }
    }
    const content = parseProfileResult(raw)
    if (!content) return { ok: false, cards: cards.length, reason: 'unparsable output' }
    const profile = store.upsertProfile(userId, content, cards.length, now)
    return { ok: true, content, version: profile?.version || 1, cards: cards.length }
  }
}
